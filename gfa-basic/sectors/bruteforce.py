  ...
  while(True):
    try:
      ...
      if newpointer > len(bites):
        raise Exception(str(newpointer) + " is beyond end of file!")
      pointer = newpointer
    except BaseException as err:
      sys.stdout.write("..stopped at byte {:d}/{:d}".format(pointer,len(bites)) + ".." + str(err) + "..")
      break

  return pointer >= len(bites)-256;
...
if __name__ == '__main__':
  ...
  sequence  = ["1242.sec","1243.sec","1244.sec","1245.sec","1246.sec","1247.sec","1248.sec"]
  sequences = []

  for sector in glob.glob("*.sec"):
    if sector in sequence:
      continue
    sys.stdout.write("testing sector " + sector)
